...
//Send command/data to SSD1306 (OLED/PLED driver)
...
void display_init(void) {
  DISPLAY_CHANGE_TO_COMMAND_MODE;
	...
	spi_send_recv(0xAE);  //Display OFF (sleep mode) 
	...
	spi_send_recv(0xA1);  //column address 127 is mapped to SEG0
	spi_send_recv(0xC8);  //remapped mode. Scan from COM[N-1] to COM0
	
	spi_send_recv(0xDA);  //Set COM Pins
	spi_send_recv(0x20);
	
	spi_send_recv(0xAF);  //Display ON in normal mode
}
